...
# %%
def predict(sent):
    sent=translator.translate(sent,src='hindi',dest='english').text
    y_pred=svm.predict(vectorizer.transform([sent]))
    if y_pred[0]==0:
        return "You have said a Negative sentence"
    elif y_pred[0]==1:
        return "You have said a Neutral sentence"
    else:
        return "You have said a Positive sentence"
        

# %%
import pickle
with open('clf.sav','wb') as f:
    pickle.dump(svm,f)
